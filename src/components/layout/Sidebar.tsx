  ...
  return (
    ...
    >
      ...
      <span className={cn('flex items-center gap-[2px] flex-1', isActive && 'text-base')}>
        {item.label}
        {isActive && (
          <span className="w-[2px] h-[18px] bg-chrome-sidebar animate-caret-blink motion-reduce:animate-none flex-shrink-0" aria-hidden="true" />
        )}
      </span>
      {item.badge && (
...
function SidebarNavGroup({ label, items, isActive, onClose }: SidebarNavGroupProps) {
  ...
  return (
    <>
      <div className="pt-4 pb-1 px-3 text-xs font-semibold text-white uppercase tracking-wider">
        {label}
      </div>
      {items.map((item) => (
        <SidebarNavItem
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      {isOpen && (
        <div
          className="fixed inset-0 z-[45] bg-neutral-text-primary/80 backdrop-blur-sm md:hidden"
          onClick={onClose}
        />
      )}
              ...
              <div className="mb-4 pb-4 border-b border-white/15 space-y-3">
                ...
                <div className="px-1">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <ContestCountdown targetTime={activeContest.endTime} className="text-left [&_p]:text-xs [&_p]:mb-0 [&_p]:text-white [&_.font-mono]:text-sm [&_.font-mono]:text-white" />
                  </div>
                </div>
                ...
                {activeContest.problems.length > 0 && (
                  <div className="px-1">
                    <p className="text-[10px] font-semibold text-white uppercase tracking-widest mb-1.5">Problemas</p>
